...
# === SYSTÈME PVP (DRAFT MODE) ===
pvp_matches = {}  # Dictionnaire des parties PvP
pvp_stats = {}    # Statistiques joueurs globales

def create_pvp_match(seed=None):
    """Crée une nouvelle partie PvP avec draft mode"""
    if seed is None:
        seed = int(time.time() * 1000) % 10000
    
    random.seed(seed)
    
    # Générer 50 brainrots identiques pour les deux joueurs
    common_pool = []
    for _ in range(50):
    ...
    # Créer les pools pour les deux joueurs (même contenu, ordre différent)
    p1_pool = common_pool.copy()
    p2_pool = common_pool.copy()
    random.shuffle(p1_pool)
    random.shuffle(p2_pool)
    
    match_id = f"pvp_{int(time.time())}_{seed}"
    
    pvp_matches[match_id] = {
        "seed": seed,
        "status": "waiting",  # waiting, p1_turn, p2_turn, finished
        "p1_name": None,
        "p2_name": None,
        "p1_pool": p1_pool,
        "p2_pool": p2_pool,
        ...
        "p1_fortune": 0,
        "p2_fortune": 0,
        "p1_available": list(range(5)),  # Indices des brainrots disponibles pour voir
        "p2_available": list(range(5)),
        "current_turn": "p1",  # p1 ou p2
...
@app.route('/test')
def test():
    """Servir la page de test des graphiques."""
...
def pvp_create():
    ...
    match_id = create_pvp_match(data.get('seed'))
    return jsonify({"match_id": match_id})

@app.route('/api/pvp/join/<match_id>', methods=['POST'])
def pvp_join(match_id):
    """Rejoindre une partie PvP"""
    if match_id not in pvp_matches:
        return jsonify({"error": "Match not found"}), 404
    
    data = request.json or {}
    player_name = data.get('player_name', f"Player_{int(time.time())}")
    match = pvp_matches[match_id]
    
    if match["p1_name"] is None:
        match["p1_name"] = player_name
        player_pos = "p1"
    elif match["p2_name"] is None:
        match["p2_name"] = player_name
        player_pos = "p2"
        match["status"] = "p1_turn"  # Commencer la partie
    else:
        return jsonify({"error": "Match is full"}), 400
    
    return jsonify({
        "player_pos": player_pos,
        "match": match,
        "available": match.get(f"p{player_pos[-1]}_available", [])
    })

@app.route('/api/pvp/state/<match_id>', methods=['GET'])
def pvp_state(match_id):
    ...
    return jsonify({
        ...
        "p1_fortune": round(match["p1_fortune"], 2),
        "p2_fortune": round(match["p2_fortune"], 2),
        "p1_picks": match["p1_picks"],
        "p2_picks": match["p2_picks"],
        "p1_available": match["p1_available"],
        "p2_available": match["p2_available"],
        "current_turn": match["current_turn"],
        "p1_available_brainrots": [match["p1_pool"][i] for i in match["p1_available"]],
        "p2_available_brainrots": [match["p2_pool"][i] for i in match["p2_available"]]
    })

@app.route('/api/pvp/pick/<match_id>/<player>', methods=['POST'])
def pvp_pick(match_id, player):
    """Choisir un brainrot pendant le draft"""
    if match_id not in pvp_matches:
        return jsonify({"error": "Match not found"}), 404
    ...
    available = match[f"{player}_available"]
    if pick_index not in available:
        return jsonify({"error": "Invalid pick"}), 400
    
    # Ajouter le pick
    picked_brainrot = match[f"{player}_pool"][pick_index]
    match[f"{player}_picks"].append(picked_brainrot)
    match[f"{player}_fortune"] += picked_brainrot["price"]
    
    # Supprimer du pool disponible
    available.remove(pick_index)
    
    # Ajouter le prochain brainrot disponible
    all_picks_count = len(match["p1_picks"]) + len(match["p2_picks"])
    next_index = 5 + all_picks_count
    if next_index < 50:
        available.append(next_index)
    
    # Alterner le tour
    if len(match["p1_picks"]) == 50 and len(match["p2_picks"]) == 50:
        match["status"] = "finished"
        winner = "p1" if match["p1_fortune"] > match["p2_fortune"] else "p2"
    else:
        match["current_turn"] = "p2" if player == "p1" else "p1"
    
    return jsonify({
        "success": True,
        "picked": picked_brainrot,
        "p1_fortune": round(match["p1_fortune"], 2),
        "p2_fortune": round(match["p2_fortune"], 2),
        "status": match["status"]
    })
...
def pvp_leaderboard():
    """Obtenir le classement des joueurs PvP"""
    # Pour l'instant, retourner un leaderboard vide
    # À étendre avec base de données
